...
def format_question(primer_desc,primer_code , question, model_type):
    ...
    return  '"""\n' + primer_desc +"user question:" + question + '\n"""\n' + primer_code
...
def get_primer(df_dataset, df_name):
    ...
    primer_desc = primer_desc + "\nUsing Python version 3.9.12, create a script using the dataframe df to graph the following: Ensure that the code that you are crafting in confined within the function create_plotly_figure(data_viz) which takes one postional argument data_viz which is a dataframe object, ensure that within the function you are including a line "+ "    df =" + df_name + ".copy() \n"
    primer_desc = primer_desc + "\njust return the function create_plotly_figure(data_viz) dont call the function "
    primer_desc = primer_desc + "\n strictly dont include lines such as the following,data_file = 'data_file.csv' or create_plotly_figure(df) .  remember ur task is just to modify the function to user's question"
    pimer_code = "import pandas as pd\nimport plotly.graph_objects as go\n"
    pimer_code = pimer_code + "def create_plotly_figure(data_viz):\n"
    pimer_code = pimer_code + "    df =" + df_name + ".copy() \n"
    pimer_code = pimer_code + "    fig = go.Figure()\n"
    pimer_code = pimer_code + "    return fig\n"
